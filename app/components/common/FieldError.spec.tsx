...
describe('FieldError', () => {
  ...
  it('should return null when message is empty string', () => {
    const { container } = renderWithProviders(<FieldError message="" />);
    expect(container.firstChild).toBeNull();
  });

  it('should render error message when provided', () => {
    renderWithProviders(<FieldError message="This field is required" />);
    expect(screen.getByText('This field is required')).toBeInTheDocument();
  });

  it('should render message as span element', () => {
    const { container } = renderWithProviders(<FieldError message="Error" />);
    const span = container.querySelector('span');
    expect(span?.tagName).toBe('SPAN');
  });

  it('should apply destructive color styles', () => {
    renderWithProviders(<FieldError message="Error message" />);
    const span = screen.getByText('Error message');
    expect(span).toHaveClass('text-destructive');
  });

  it('should apply text styling classes', () => {
    renderWithProviders(<FieldError message="Error" />);
  ...
  it('should support markdown in error messages', () => {
    renderWithProviders(<FieldError message="This is **bold** error" />);
    expect(screen.getByText(/bold/i)).toBeInTheDocument();
  });

  it('should render markdown links in error messages', () => {
    renderWithProviders(
      <FieldError message="Error: [click here](https://example.com)" />
    );
  ...
  it('should apply link hover styling', () => {
    renderWithProviders(<FieldError message="[link](https://example.com)" />);
  ...
  it('should apply link underline styling', () => {
    renderWithProviders(<FieldError message="[text](https://example.com)" />);
  ...
  it('should accept and apply custom className', () => {
    renderWithProviders(
      <FieldError message="Error" className="custom-error-class" />
    );
    const span = screen.getByText('Error');
  ...
  it('should merge custom className with default classes', () => {
    renderWithProviders(<FieldError message="Error" className="mt-2" />);
    const span = screen.getByText('Error');
  ...
  it('should handle special characters in error message', () => {
    ...
    renderWithProviders(<FieldError message={specialMessage} />);
  ...
  it('should handle long error messages without truncation', () => {
    ...
    renderWithProviders(<FieldError message={longMessage} />);

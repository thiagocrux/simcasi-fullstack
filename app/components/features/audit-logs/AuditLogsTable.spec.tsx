...
describe('AuditLogsTable', () => {
  ...
  it('should render component.', () => {
    const { container } = renderWithProviders(<AuditLogsTable />);
    expect(container).toBeInTheDocument();
  });
  it('should accept pageSize prop.', () => {
    const { container } = renderWithProviders(<AuditLogsTable pageSize={50} />);
    expect(container).toBeInTheDocument();
  });

  it('should accept showFilterInput prop.', () => {
    const { container } = renderWithProviders(
      <AuditLogsTable showFilterInput={false} />
    );
    expect(container).toBeInTheDocument();
  });
  ...
  it('should accept showColumnToggleButton prop.', () => {
    const { container } = renderWithProviders(
      <AuditLogsTable showColumnToggleButton={false} />
    );
    expect(container).toBeInTheDocument();
  });
  ...
  it('should accept showPrintButton prop.', () => {
    const { container } = renderWithProviders(
      <AuditLogsTable showPrintButton={false} />
    );
    expect(container).toBeInTheDocument();
  });
